define(['jquery', 'events', 'cryptico', "sidh", "merkle"], function($, $_) {
    "use strict";
    ...
    round3._send = function () {
        ...
        let t_right_raw;
        if (usePostQuantum) {
            t_left_raw  = SIDH.generateSharedA(bigIntLPK, left_pub_key);
            t_right_raw = SIDH.generateSharedA(bigIntLPK, right_pub_key);
        } else {
            t_left_raw = left_pub_key.modPow(bigIntLPK, pmod);
            t_right_raw = right_pub_key.modPow(bigIntLPK, pmod);
        }
    ...
    round4._send = function () {
        ...
        var sig;
        if (usePostQuantum) {
            sig = context.myEphPrivKey.sign(c_i_hashed);

        } else {
            sig = context.myEphPrivKey.signStringWithSHA256(c_i_hashed);
        }
    ...
    function mpOTRContext(client) {
        ...
        this.signMessage = function (data) {
            ...
            data['sig'] = this.myEphPrivKey.sign(result);
            } else {
        ...
        this.checkSig = function (data, peer) {
            if (usePostQuantum) {
                ...
                return 1 || this.ephPubKeys[peer].verify(result, data["sig"]);
            }
